  ...
  def index
    ...
    if (user.id == current_user.id and user.role == 0)
      ...
      if current_user.role == 0
        render json: {status: "Unauthorized", message: "You are not authorised to access the products of other sellers products"},status: :unauthorized
      else
        render json: {status: "Unauthorized", message: "Buyers are not authorised for this feature"},status: :unauthorized
      end
    end
  end
